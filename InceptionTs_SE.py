...
def Inception_module(inputs,filters):
    ...
    layer_20 = keras.layers.Conv1D(filters[0],5, padding='same', activation='relu',use_bias=False)(inputs)
    layer_40 = keras.layers.Conv1D(filters[1],3, padding='same', activation='relu',use_bias=False)(inputs)
...
def build_model(inputShape, InceptionLayers, MLPlayers, dropoutRate = 0.2, filters = [100,64],residuals = True):
    ...
    for i in range(InceptionLayers):
        ...
        if (i %3 == 2) and (residuals == True):
            x = residual_connection(x,init_inputs)
            init_inputs = x
...
def se_block(in_block, ch, ratio=16):
    x = keras.layers.GlobalAveragePooling1D()(in_block)
